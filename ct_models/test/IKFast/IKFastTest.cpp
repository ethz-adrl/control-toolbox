...
TEST(HyAIKTest, IKFastTest)
{
    ...
    for (int i = 0; i < 100; ++i)
    {
        ...
        typename ct::rbd::HyAInverseKinematics<double>::JointPositionsVector_t solutions;
        bool solutionFound = hya_ik_solver.computeInverseKinematics(solutions, ee_pose);
        if (solutionFound)
        {
