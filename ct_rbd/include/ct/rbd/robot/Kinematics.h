/**********************************************************************************************************************
This file is part of the Control Toolbox (https://adrlab.bitbucket.io/ct),
copyright by ETH Zurich, Google Inc.
 ...
 */
template <class RBD, size_t N_EE> class Kinematics {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  Kinematics(
      std::shared_ptr<RBD> rbdContainer = std::shared_ptr<RBD>(new RBD()))
  ...
  void initEndeffectors(
      std::array<EndEffector<NJOINTS, SCALAR>, NUM_EE> &endeffectors) {
  ...
  const HomogeneousTransforms &transforms() const {
    return robcogen().homogeneousTransforms();
  }
  ...
  //	RigidBodyPoseTpl<SCALAR> getLinkPoseInWorld(size_t linkId, const
  //typename JointStateTpl<NJOINTS,
  // SCALAR>::Position& jointPosition, const RigidBodyPoseTpl<SCALAR>& basePose)
  // {
  //		throw std::runtime_error("not implemented");
  //		return getHomogeneousTransformBaseLinkById(linkId,
  //jointPosition);
  //	};

  Velocity3Tpl
  getEEVelocityInBase(size_t eeId,
                      const tpl::RBDState<NJOINTS, SCALAR> &rbdState) {
    Velocity3Tpl eeVelocityBase;
    eeVelocityBase.toImplementation() =
        (robcogen().getJacobianBaseEEbyId(eeId, rbdState.jointPositions()) *
         rbdState.jointVelocities())
    ...
    eeVelocityBase +=
        rbdState.base().velocities().getRotationalVelocity().cross(
  ...
  Velocity3Tpl
  getEEVelocityInWorld(size_t eeId,
                       const tpl::RBDState<NJOINTS, SCALAR> &rbdState) {
  ...
  /*!
   * Computes the forward kinematics for the end-effector position and expresses
   * the end-effector position in robot
   * base coordinates.
   ...
   */
  Position3Tpl
  getEEPositionInBase(size_t eeID,
                      const typename tpl::JointState<NJOINTS, SCALAR>::Position
                          &jointPosition) {
  ...
  /*!
   * Computes the forward kinematics for the end-effector position and expresses
   * the end-effector pose in robot base
   * coordinates.
   * @param eeID unique identifier of the end-effector in question
   * @param jointPosition current robot joint positions
  ...
  /*!
   * compute the forward kinematics and return a rotation matrix specifying the
   * ee-rotation w.r.t. the base frame
   */
  Matrix3Tpl
  getEERotInBase(size_t eeID,
                 const typename tpl::JointState<NJOINTS, SCALAR>::Position
                     &jointPosition) {
   ...
   */
  Position3Tpl
  getEEPositionInWorld(size_t eeID, const RigidBodyPoseTpl &basePose,
                       const typename tpl::JointState<NJOINTS, SCALAR>::Position
                           &jointPosition) {
    ...
    // vector from origin to endeffector = vector from origin to base + vector
    // from base to endeffector
    return basePose.position() + W_x_EE;
  }

  //! get the end-effector pose in world coordinates
  RigidBodyPoseTpl
  getEEPoseInWorld(size_t eeID, const RigidBodyPoseTpl &basePose,
                   const typename tpl::JointState<NJOINTS, SCALAR>::Position
                       &jointPosition) {
    ...
    Position3Tpl W_p_EE =
        basePose.template rotateBaseToInertia(B_x_EE.position());

    // orientation rotated into world frame
    QuaterionTpl B_q_EE = B_x_EE.getRotationQuaternion();
    QuaterionTpl W_q_EE =
        basePose.template rotateBaseToInertiaQuaternion(B_q_EE);
  ...
  Matrix3Tpl
  getEERotInWorld(size_t eeID, const RigidBodyPoseTpl &basePose,
                  const typename tpl::JointState<NJOINTS, SCALAR>::Position
                      &jointPosition) {
  ...
  void addIKSolver(
      const std::shared_ptr<InverseKinematicsBase<NJOINTS, SCALAR>> &solver,
      size_t eeID, size_t solverID = 0) {
    if (solverID >= 100)
  ...
  std::shared_ptr<InverseKinematicsBase<NJOINTS, SCALAR>>
  getIKSolver(const size_t eeID, const size_t solverID = 0) const {
    if (solverID >= 100)
      throw "Solver ID must be less than 100.";
    size_t hash = eeID * 100 + solverID;
  ...
  /**
   * \brief Transforms a force applied at an end-effector and expressed in the
   * world into the link frame the EE is
  ...
  EEForce mapForceFromWorldToLink3d(
      const Vector3Tpl &W_force, const RigidBodyPoseTpl &basePose,
  ...
  EEForce mapForceFromWorldToLink(
      const EEForce &W_force, const RigidBodyPoseTpl &basePose,
      const typename tpl::JointState<NJOINTS, SCALAR>::Position &jointPosition,
      size_t eeId) {
    // get endeffector position in world
    Position3Tpl B_x_EE = getEEPositionInBase(eeId, jointPosition);

    return mapForceFromWorldToLink(W_force, basePose, jointPosition, B_x_EE,
                                   eeId);
  }
  ...
  EEForce mapForceFromWorldToLink(
      const EEForce &W_force, const RigidBodyPoseTpl &basePose,
      const typename tpl::JointState<NJOINTS, SCALAR>::Position &jointPosition,
      const Position3Tpl &B_x_EE, size_t eeId) {
    // get the link that the EE is attached to
    size_t linkId = getEndEffector(eeId).getLinkId();

    // transform the force/torque to an equivalent force/torque in the base
    // using a lever-arm for the torque
    EEForce B_force;

    B_force.force() =
        basePose.template rotateInertiaToBase<Vector3Tpl>(W_force.force());
    B_force.torque() =
        B_x_EE.toImplementation().cross(B_force.force()) +
        basePose.template rotateInertiaToBase<Vector3Tpl>(W_force.torque());

    // transform force to link on which endeffector sits on
    return EEForce(
        robcogen().getForceTransformLinkBaseById(linkId, jointPosition) *
        B_force);
  }

  /**
   * \brief Transforms a force applied at an end-effector expressed in an
   * arbitrary (end-effector)
   * frame into the link frame the EE is rigidly connected to.
   *
   * This function does not assume any position or orientation of the
   * end-effector. Therefore,
   * the user must specify the orientation of the end-effector in the base.
   * There are two common
   * choices: use zero rotation to assume forces/torques are expressed in the
   * base. Or use the
   * base orientation to assume the end-effector forces/torques are expressed in
   * the world. For
   * this variant, also see mapForceFromWorldToLink().
   *
   * NOTE: Even if zero or base rotation is assumed, you have to pass the
   * correct position of the
   * end-effector expressed in the base as part of T_B_EE! Do NOT pass the base
   * pose directly here!
   *
  ...
  EEForce mapForceFromEEToLink(
      const EEForce &EE_force, const RigidBodyPoseTpl &T_B_EE,
      const typename tpl::JointState<NJOINTS, SCALAR>::Position &jointPosition,
      size_t eeId) {
    // get the link that the EE is attached to
    size_t linkId = getEndEffector(eeId).getLinkId();
    ...
    B_force.force() =
        T_B_EE.template rotateBaseToInertia<Vector3Tpl>(EE_force.force());

    // build the cross product and add the torque (which first is rotated from
    // the end-effector to the base)
    B_force.torque() =
        B_x_EE.cross(B_force.force()) +
        T_B_EE.template rotateBaseToInertia<Vector3Tpl>(EE_force.torque());
