...
RBDDataMap<T , N>::RBDDataMap(const T& value) {
    assigndata(value);
}

template<typename T , size_t N> inline
RBDDataMap<T , N>::RBDDataMap(const RBDDataMap& rhs)
{
    copydata(rhs);
}

template<typename T , size_t N> inline
RBDDataMap<T, N>& RBDDataMap<T , N>::operator=(const RBDDataMap& rhs)
{
    if(&rhs != this) {
        copydata(rhs);
    }
...
template<typename T,size_t N> inline
RBDDataMap<T , N>& RBDDataMap<T,N>::operator=(const T& value)
{
    assigndata(value);
    return *this;
}
...
template<typename T,size_t N> inline
T& RBDDataMap<T,N>::operator[](size_t l) {
    return data[l];
}

template<typename T, size_t N> inline
const T& RBDDataMap<T,N>::operator[](size_t l) const {
    return data[l];
}
