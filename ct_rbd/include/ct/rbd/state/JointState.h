 ...
 */
template <size_t NJOINTS, typename SCALAR = double> class JointState {
public:
  ...
  typedef Eigen::VectorBlock<const joint_state_vector_t, NJOINTS>
  ...
  inline JointState<NJOINTS, SCALAR>
  operator+(const JointState<NJOINTS, SCALAR> &rhs) const {
  ...
  bool isApprox(const JointState<NJOINTS, SCALAR> &rhs,
  ...
  const JointPositionBlockConst getPositions() const {
    return state_.template head<NJOINTS>();
  }
  ...
  template <typename T> bool checkPositionLimits(T lowerLimit, T upperLimit) {
    assert(lowerLimit.size() == NJOINTS && upperLimit.size() == NJOINTS &&
  ...
  const JointPositionBlockConst getVelocities() const {
    return state_.template tail<NJOINTS>();
  }
  ...
  template <typename T> bool checkVelocityLimits(T limit) {
...
template <size_t NJOINTS> using JointState = tpl::JointState<NJOINTS, double>;
