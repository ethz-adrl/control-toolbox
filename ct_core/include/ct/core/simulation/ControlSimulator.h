...
public:
  ...
  //! constructor
  ControlSimulator(Time sim_dt, Time control_dt,
  ...
  ControlSimulator(const ControlSimulator& arg)
    ...
    if (arg.controller_) {
      controller_ =
          std::shared_ptr<Controller<STATE_DIM, CONTROL_DIM, SCALAR>>(arg.controller_->clone());
      system_->setController(controller_);
    }
  }
...
protected:
  ...
  virtual void simulateSystem(Time duration) {
    ...
    while (
        std::chrono::duration<double>(std::chrono::high_resolution_clock::now() - sim_start_time_)
            .count() < duration && !stop_) {
      ...
      while (std::chrono::duration<double>(std::chrono::high_resolution_clock::now() - wall_time)
                 .count() < control_dt_)
        usleep(100);
