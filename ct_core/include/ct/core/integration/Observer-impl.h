...
Observer<MANIFOLD>::Observer(const EventHandlerPtrVector& eventHandlers)
...
template <typename MANIFOLD>
void Observer<MANIFOLD>::observe(const MANIFOLD& x, const SCALAR& t)
{
    for (size_t i = 0; i < eventHandlers_.size(); i++)
    {
...
template <typename MANIFOLD>
void Observer<MANIFOLD>::log(const MANIFOLD& x, const SCALAR& t)
{
    states_.push_back(x);
    times_.push_back(t);
}
...
template <typename MANIFOLD>
void Observer<MANIFOLD>::observeInternal(const MANIFOLD& x, const SCALAR& t)
{
    for (size_t i = 0; i < eventHandlers_.size(); i++)
    {
        if (eventHandlers_[i]->callOnSubsteps() && eventHandlers_[i]->checkEvent(x, t))
