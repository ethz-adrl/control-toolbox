...
int main(int argc, char **argv)
{
    ...
    std::shared_ptr<LQOCProblem<MySys::STATE_DIM, MySys::CONTROL_DIM>> lqocProblem(
        new LQOCProblem<MySys::STATE_DIM, MySys::CONTROL_DIM>(N));
    ...
    for (int t = dm_seq.getStartTimeFromIdx(0); t < dm_seq.getTotalDuration(); t++)
    {
        ...
        lqocProblem->B_[t] = B;
        lqocProblem->b_[t] =
            A * x0 + B * u0 - x0;  // To achieve relative formulation: d_x(t+1) = A(t)*d_x(t) + B(t)*d_u(t) + b(t)
    ...
    // SOLVE
    std::shared_ptr<LQOCSolver<MySys::STATE_DIM, MySys::CONTROL_DIM>> solver(
        new HPIPMInterface<MySys::STATE_DIM, MySys::CONTROL_DIM>);
    ...
    for (int t = dm_seq.getStartTimeFromIdx(0); t < dm_seq.getTotalDuration(); t++)
    {
        ...
        std::cout << "t: " << t << "\tx: " << xSol[t].transpose() << "\tu: " << uSol[t].transpose() << std::endl;
    }
    std::cout << "t: " << dm_seq.getTotalDuration() << "\tx: " << xSol[dm_seq.getTotalDuration()].transpose()
              << std::endl;
}