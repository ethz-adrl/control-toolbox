...
double CostEvaluatorSimple<STATE_DIM, CONTROL_DIM>::eval()
{
	...
	{
		costFct_->setCurrentStateAndControl(w_->getState(i), w_->getControl(i));
		cost += phi_(i)*costFct_->evaluateIntermediate();
	}

	costFct_->setCurrentStateAndControl(w_->getState(settings_.N_), control_vector_t::Zero());
	...
	for (size_t i = 0; i< settings_.N_ + 1; ++i)
	{
		...
		if(settings_.objectiveType_ == DmsSettings::OPTIMIZE_GRID && i < settings_.N_)
		{
			if(settings_.splineType_ == DmsSettings::ZERO_ORDER_HOLD)
			{
				...
				size_t idx = w_->getShotDurationIndex(i);
				grad(idx) = dJdH;
			}
			...
			else if(settings_.splineType_ == DmsSettings::PIECEWISE_LINEAR)
			{
				...
				costFct_->setCurrentStateAndControl(w_->getState(i+1), w_->getControl(i+1));
				dJdH += 0.5 * costFct_->evaluateIntermediate();
				grad(w_->getShotDurationIndex(i)) = dJdH;
			}
		}	
	}
...
void CostEvaluatorSimple<STATE_DIM, CONTROL_DIM>::updatePhi()
	{
		// double curr_total_time = w_->getTimeGridPtr()->getTtotal();
		...
		case DmsSettings::ZERO_ORDER_HOLD:
		{
			...
			{
				phi_(i) = (w_->getShotDuration(i)); // / curr_total_time;
			}
		...
		case DmsSettings::PIECEWISE_LINEAR:
		{
			...
			phi_(0) = 0.5 * (w_->getShotDuration(0)); // / curr_total_time;

			// set weights for s_1, ..., s_(N-1)
			for(size_t i = 1; i < settings_.N_; i++)
			{
				phi_(i) = 0.5 * (w_->getShotDuration(i) + w_->getShotDuration(i-1));
				// phi_(i) = w_->getTimeGridPtr()->getShotDuration(i);
										/// curr_total_time;
			}

			// set special weight for s_N
			phi_(settings_.N_) = 0.5 * (w_->getShotDuration(settings_.N_-1)) ; // / curr_total_time;
